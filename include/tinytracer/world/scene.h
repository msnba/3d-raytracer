...
public:
  ...
  template <typename T_> T_ *add(std::unique_ptr<T_> object) {
    static_assert(std::is_base_of_v<Object, T_>,
                  "Object must derive from Object");
  ...
  template <typename T_> std::vector<T_ *> getObjectsOfType() const {
    std::vector<T_ *> output;
    for (const std::unique_ptr<Object> &obj : objects_)
      if (T_ *casted = dynamic_cast<T_ *>(
              obj.get())) // i had no idea you can instantiate variables in if
                          // statements
