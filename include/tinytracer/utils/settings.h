...
public:
  ...
  template <typename T_> void setValue(const std::string &key, T_ value) {
  ...
  T_ getValue(const std::string &key, T_ fallback) const {
    ...
    throw std::runtime_error("Key \"" + key + "\" exists, but wrong type \"" +
                             typeid(T_).name() + "\" requested.");
  }
