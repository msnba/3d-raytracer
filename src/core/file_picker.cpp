...
FilePicker::FilePicker() : isShutdown_(false) {
  thread_ = std::thread(
      &FilePicker::workerLoop,
      this); // initializes the file save handler in a seperate thread
}
...
std::string FilePicker::ensureExtension(std::string path,
                                        const std::string &ext) {
  const std::string dotExt = "." + ext;
  if (path.size() < dotExt.size() ||
      path.compare(path.size() - dotExt.size(), dotExt.size(), dotExt) != 0)
    path += dotExt;
...
std::string
FilePicker::query(const Entry &entry, bool isSave,
...
void FilePicker::workerLoop() {
  std::unique_lock<std::mutex> lock(
      mutex_); // allows for manual locking/unlocking
  while (!isShutdown_ || !entries_.empty()) {
    cv_.wait(lock, [this] { return isShutdown_ || !entries_.empty(); });
...
std::string FilePicker::runDialog(const QueuedEntry &qe) {
  const std::string title = qe.entry.dialogTitle.empty()
                                ? qe.isSave ? "Save File" : "Open File"
                                : qe.entry.dialogTitle;
  ...
  if (qe.isSave) {
    const std::string defaultName = qe.entry.defaultFilename.empty()
                                        ? ("file." + qe.entry.fileExtension)
                                        : qe.entry.defaultFilename;

    std::string destination =
        pfd::save_file(title, defaultName, {label, glob}).result();
