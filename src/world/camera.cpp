...
Camera::Camera(float fov, float speed, float yaw, float pitch,
               glm::vec3 cameraPos)
    : fov_(fov), speed_(speed), yaw_(yaw), pitch_(pitch), cameraPos_(cameraPos),
...
bool Camera::handleKeyInput(GLFWwindow *window, float deltaTime) {
  ...
  if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
    cameraPos_ -=
        velocity * glm::normalize(glm::cross(cameraFront_, glm::vec3(0, 1, 0)));
    isMoving = true;
  }
...
bool Camera::handleMouseInput(uint32_t &accumFrameIndex, float mouseX,
                              float mouseY, float &mouseLastX,
                              float &mouseLastY) {
...
bool Camera::handleScrollInput(double xoffset, double yoffset) {
  ...
  fov_ = std::min(
      std::max(fov_ - sensitivity * static_cast<float>(yoffset), 10.f), 150.f);
