...
static glm::vec4 readVec4(const rapidjson::Value &v) {
  return glm::vec4(v[0].GetFloat(), v[1].GetFloat(), v[2].GetFloat(),
                   v[3].GetFloat());
}
...
bool Scene::saveToFile(const std::string &filepath) const {
  if (filepath.empty()) {
    tinytracer::utils::error("Filepath is false.");
    return false;
  }
#ifdef _WIN32
...
#endif
  if (!fp) {
    tinytracer::utils::error("File not initialized correctly to write to.");
    return false;
  }
...
bool Scene::remove(Object *target) {
  ...
  auto iterator = std::find_if(objects_.begin(), objects_.end(),
